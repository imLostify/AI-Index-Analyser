...
def display_probabilities(probabilities, targets, language='de'):
    ...
    with col1:
        ...
        fig = go.Figure(data=[
            ...
            go.Bar(name='Neutral', x=[get_text('probability', language)], y=[neutral_prob],
        ...
        if total > 0:
            ...
            **{get_text('signal_analysis', language)}:**
            ...
            - Neutrale Signale: {neutral}/{total} ({neutral/total*100:.1f}%)
...
def main():
    ...
    with st.sidebar:
        ...
        with col1:
            if st.button("1 Woche", use_container_width=True):
                start_date = datetime.now().date() - timedelta(days=7)
                end_date = datetime.now().date()
                st.rerun()
        with col2:
            if st.button("1 Monat", use_container_width=True):
                start_date = datetime.now().date() - timedelta(days=30)
                end_date = datetime.now().date()
                st.rerun()
        with col3:
            if st.button("1 Jahr", use_container_width=True):
        ...
        # Chart-Einstellungen
        st.markdown(f"### Chart-Einstellungen")
        
        # VWAP Toggle
        show_vwap = st.checkbox(
            "VWAP anzeigen",
            value=st.session_state.get('show_vwap', False),
            help="Volume Weighted Average Price im Hauptchart anzeigen"
        )
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tab1:
            ...
            if 'analysis_date' in data:
                st.caption(f"Analyse vom: {data['analysis_date']}")
        ...
        with tab2:
            ...
            # Bedienungshinweise
            with st.expander("💡 Chart-Bedienung & Legende", expanded=False):
                if lang == 'de':
                    st.markdown("""
                    **Zoom & Navigation:**
        ...
        with tab3:
            ...
            if data.get('indicators'):
                ...
                st.info("Keine Indikatoren berechnet")
        ...
        with tab4:
            ...
            if patterns_data and patterns_data.get('patterns'):
                ...
                st.info(f"Keine {get_text('patterns_found', lang).lower()}")
                ...
                try:
                    ...
                    with col1:
                        with st.spinner(get_text('ai_thinking', lang)):
                            ...
                            st.info(f"Max Tokens für Berichte: {current_max_tokens}")
                            ...
                            if st.button(get_text('generate_report', lang)):
                                try:
                                    with st.spinner(get_text('generating_report', lang)):
                                        ...
                                        # Übergebe max_tokens an die Funktion
